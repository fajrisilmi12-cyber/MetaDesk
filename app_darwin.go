...
static void configureWebKitMemoryLimits(void) {
    ...
    dispatch_once(&onceToken, ^{
        ...
        NSString *cachePath = [paths.firstObject stringByAppendingPathComponent:@"com.metadesk/WebCache"];
...
static const long long kMetaDeskCacheCapBytes = 512LL * 1024 * 1024;
...
static long long whatsappDeskDiskCacheBytes(void) {
    @autoreleasepool {
        long long total = 0;
        NSArray* homes = @[
            [NSHomeDirectory() stringByAppendingPathComponent:@"Library/Caches/com.metadesk"],
            [NSHomeDirectory() stringByAppendingPathComponent:@"Library/WebKit/com.metadesk"],
        ];
...
static void enforceDiskCacheCap(void) {
    dispatch_async(dispatch_get_global_queue(QOS_CLASS_UTILITY, 0), ^{
        @autoreleasepool {
            ...
            if (bytes <= kMetaDeskCacheCapBytes) return;
            ...
            [[NSFileManager defaultManager] removeItemAtPath:
                [NSHomeDirectory() stringByAppendingPathComponent:@"Library/Caches/com.metadesk"]
                                                          error:nil];
            [[NSFileManager defaultManager] removeItemAtPath:
                [NSHomeDirectory() stringByAppendingPathComponent:@"Library/WebKit/com.metadesk/NetworkCache"]
                                                          error:nil];
        }
    });
}
    ...
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        if (wv) {
            ...
            [wv evaluateJavaScript:wrapped completionHandler:^(id result, NSError *error) {
                if (error) {
                    NSLog(@"[MetaDesk] JS evaluation error: %@ for script: %@", error, script);
                }
            }];
        } else {
            NSLog(@"[MetaDesk] Error: WKWebView not found for script: %@", script);
        }
    });
}
...
static void openDownloadsFolderNative(void) {
    @autoreleasepool {
        ...
        NSString *targetFolder = [downloads stringByAppendingPathComponent:@"MetaDesk Downloads"];

        NSString *support = [NSSearchPathForDirectoriesInDomains(NSApplicationSupportDirectory, NSUserDomainMask, YES) firstObject];
        NSString *settingsFile = [support stringByAppendingPathComponent:@"MetaDesk/settings.json"];
...
static void setupStatusItem(void) {
    @autoreleasepool {
        ...
        if (button) {
            ...
            [button setToolTip:@"MetaDesktop"];
        }

        NSMenu* trayMenu = [[NSMenu alloc] initWithTitle:@"WhatsApp Tray"];

        NSMenuItem* appTitle = [trayMenu addItemWithTitle:@"MetaDesk" action:nil keyEquivalent:@""];
        ...
        [trayMenu addItemWithTitle:@"Quit MetaDesk" action:@selector(terminate:) keyEquivalent:@"q"];
...
static void setupMacOSMenuBar(void) {
    @autoreleasepool {
        ...
        NSMenuItem* aboutItem = [appMenu addItemWithTitle:@"About MetaDesk" action:@selector(orderFrontStandardAboutPanel:) keyEquivalent:@""];
        ...
        [appMenu addItemWithTitle:@"Hide MetaDesk" action:@selector(hide:) keyEquivalent:@"h"];
        ...
        [appMenu addItem:[NSMenuItem separatorItem]];
        [appMenu addItemWithTitle:@"Quit MetaDesk" action:@selector(terminate:) keyEquivalent:@"q"];
...
func getUserDataDir() string {
	...
	dir := filepath.Join(home, "Library", "Application Support", "MetaDesk", "UserData")
...
func checkSingleInstance() (*os.File, bool) {
	...
	if err != nil {
		_ = exec.Command("osascript", "-e", `tell application "MetaDesk" to activate`).Run()
		return nil, false
	}
	return file, true
}
...
func getAppBundlePath() string {
	...
	if err != nil {
		return "/Applications/MetaDesk.app"
	}
	if idx := strings.Index(execPath, ".app"); idx != -1 {
		return execPath[:idx+4]
	}
	if _, err := os.Stat("/Applications/MetaDesk.app"); err == nil {
		return "/Applications/MetaDesk.app"
	}
	return execPath
}
...
func getLaunchAgentPath() string {
	...
	return filepath.Join(home, "Library", "LaunchAgents", "com.metadesk.plist")
}
...
func isAutoStartMac() bool {
	...
	return exec.Command("/bin/launchctl", "print", launchAgentDomain()+"/com.metadesk").Run() == nil
}
...
func runApp() {
	...
	_ = w.Bind("switchServiceNative", func(id string) string {
		return serviceURL(id)
	})
	w.Init(getInitScript(userAgent))
	w.Navigate(serviceURL(defaultServiceID))
