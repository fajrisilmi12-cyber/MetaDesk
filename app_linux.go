...
static void setMetaDeskGTKTheme(int mode) {
...
static void setMetaDeskKeepAbove(int enable) {
	GtkWindow* top = NULL;
...
#define TRAY_BUS_NAME "org.kde.StatusNotifierItem-metadesk"
...
static GVariant* tray_get_property(GDBusConnection* conn, const gchar* sender, const gchar* object_path, const gchar* interface, const gchar* property, GError** error, gpointer user_data) {
	...
	if (strcmp(interface, "org.kde.StatusNotifierItem") == 0) {
		if (strcmp(property, "Id") == 0) {
			return g_variant_new_string("metadesk");
		}
		...
		if (strcmp(property, "ToolTip") == 0) {
			...
			return g_variant_new("(s@a(iiay)ss)", "metadesk", empty, "MetaDesk", tip);
		}
	}
...
static void tray_update_overlay_icon(int count) {
	...
	if (count > 0) {
		g_has_overlay = 1;
		snprintf(g_overlay_icon_name, sizeof(g_overlay_icon_name), "metadesk-unread-%d", count);
	} else {
		g_has_overlay = 0;
...
func getUserDataDir() string {
	...
	dir := filepath.Join(configDir, "MetaDesk")
...
func showNativeNotification(title, message, iconPath string) {
	if iconPath != "" {
		_ = exec.Command("notify-send", "-a", "MetaDesk", "-i", iconPath, title, message).Run()
	} else {
		_ = exec.Command("notify-send", "-a", "MetaDesk", title, message).Run()
	}
}
...
func toggleAlwaysOnTopLinux() bool {
	...
	C.setMetaDeskKeepAbove(C.int(b2i(isAlwaysOnTopLinux)))
...
func getAutoStartDesktopPath() string {
	...
	return filepath.Join(home, ".config", "autostart", "metadesk.desktop")
}
...
Version=1.0
Name=MetaDesk
Comment=Lightweight MetaDesktop Client
Exec=%s
Icon=metadesk
Terminal=false
...
func runApp() {
	...
	if !isSingle {
		fmt.Println("MetaDesk is already running.")
		os.Exit(0)
	}
	if lockFile != nil {
	...
	home, _ := os.UserHomeDir()
	linuxCacheHomes = []string{
		filepath.Join(home, ".cache", "metadesk"),
		filepath.Join(home, ".cache", "MetaDesk"),
