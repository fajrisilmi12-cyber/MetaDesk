#!/usr/bin/env bash
# Unified build dispatcher for MetaDesk.
...
VERSION="${2:-1.0.0}"  # single source of truth, injected via -X main.appVersion
TARGET="${1:-all}"

export METADESK_VERSION="${VERSION}"
...
build_windows() {
    ...
    GOOS=windows GOARCH=amd64 CGO_ENABLED="${CGO_FLAG}" \
        ...
        -trimpath -o MetaDesk.exe .
    rm -f MetaDesk-Windows-x64.zip
    zip -q MetaDesk-Windows-x64.zip MetaDesk.exe
    echo "Created: MetaDesk.exe and MetaDesk-Windows-x64.zip"
    ...
    if command -v makensis >/dev/null 2>&1 && [ -f "installer/windows/MetaDesk.nsi" ]; then
        echo "Building Windows Setup installer (NSIS)..."
        rm -f "MetaDesk-Windows-x64-Setup.exe"
        case "$(uname -s)" in
            MINGW*|MSYS*|CYGWIN*) makensis "/DVERSION=${VERSION}" "installer/windows/MetaDesk.nsi" ;;
            *) makensis "-DVERSION=${VERSION}" "installer/windows/MetaDesk.nsi" ;;
        esac
        echo "Created: MetaDesk-Windows-x64-Setup.exe"
    else
