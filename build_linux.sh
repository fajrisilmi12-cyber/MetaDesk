...
APP_NAME="metadesk"
DISPLAY_NAME="MetaDesk"
VERSION="${1:-${METADESK_VERSION:-1.0.0}}"
OUTPUT_DIR="dist_linux"

# WebKitGTK variant selector (opt-in, default unchanged).
#   METADESK_WEBKIT=4.0  -> historical behavior: link against webkit2gtk-4.0
#                          (Ubuntu 22.04 Jammy and older Debian).
#   METADESK_WEBKIT=4.1  -> link against webkit2gtk-4.1 / libsoup-3.0
...
# build with METADESK_WEBKIT=4.1. Related to issues #8 and #9.
WEBKIT_VARIANT="${METADESK_WEBKIT:-4.0}"
case "${WEBKIT_VARIANT}" in
    4.0|4.1) ;;
    *) echo "error: METADESK_WEBKIT must be 4.0 or 4.1, got '${WEBKIT_VARIANT}'" >&2; exit 2 ;;
esac
...
if command -v pkg-config >/dev/null 2>&1; then
    if [ "${WEBKIT_VARIANT}" = "4.1" ]; then
        if ! pkg-config --exists gtk+-3.0 webkit2gtk-4.1; then
            echo "Warning: GTK3 and WebKit2GTK-4.1 development headers are required for METADESK_WEBKIT=4.1."
        ...
        if ! pkg-config --exists gtk+-3.0 webkit2gtk-4.0 && ! pkg-config --exists webkit2gtk-4.1; then
            ...
            echo "Debian/Ubuntu (24.04+): sudo apt-get install -y libgtk-3-dev libwebkit2gtk-4.1-dev (then build with METADESK_WEBKIT=4.1)"
            echo "Fedora/RHEL:   sudo dnf install -y gtk3-devel webkit2gtk4.1-devel"
        fi
        ...
        # the transitional Depends suggest 4.0. Prefer METADESK_WEBKIT=4.1
        ...
        if ! pkg-config --exists webkit2gtk-4.0 && pkg-config --exists webkit2gtk-4.1; then
            ...
            echo "Note: building default 4.0 path on a 4.1-only host via pc alias; for Ubuntu 24.04+/Mint 22.x use METADESK_WEBKIT=4.1 instead."
        fi
    fi
fi

if [ "${METADESK_USE_EXISTING_OUTPUT:-0}" = "1" ]; then
...
Version=1.0
Name=${DISPLAY_NAME}
Comment=Lightweight MetaDesk Client
Exec=${APP_NAME}
Icon=${APP_NAME}
Terminal=false
Categories=Network;InstantMessaging;Chat;
StartupWMClass=${APP_NAME}
Keywords=MetaDesk;Chat;Messenger;
EOF
fi

# Create tar.gz release bundle
echo "Creating release tarball (MetaDesk-Linux-${BUNDLE_ARCH}${ARTIFACT_SUFFIX}.tar.gz)..."
TAR_BUNDLE="MetaDesk-Linux-${BUNDLE_ARCH}${ARTIFACT_SUFFIX}.tar.gz"
rm -f "${TAR_BUNDLE}"
...
Section: net
Priority: optional
Description: Lightweight MetaDesk Client
 Independent, fast, and privacy-focused Meta services desktop application
 built with Go and native WebKit.
EOF

    dpkg-deb --build "${DEB_DIR}" "metadesk_${VERSION}_${DEB_ARCH}${ARTIFACT_SUFFIX}.deb"
    cp "metadesk_${VERSION}_${DEB_ARCH}${ARTIFACT_SUFFIX}.deb" "MetaDesk-Linux-${DEB_ARCH}${ARTIFACT_SUFFIX}.deb"
    rm -rf "${DEB_DIR}"
    echo "Created: metadesk_${VERSION}_${DEB_ARCH}${ARTIFACT_SUFFIX}.deb and MetaDesk-Linux-${DEB_ARCH}${ARTIFACT_SUFFIX}.deb"
fi
...
if command -v rpmbuild >/dev/null 2>&1 && { [ "${METADESK_BUILD_RPM:-0}" = "1" ] || [ -f /etc/fedora-release ]; }; then
...
Version:        ${VERSION}
Release:        1%{?dist}
Summary:        Lightweight Meta services desktop client
License:        MIT
...
%description
Independent Meta services desktop client built with Go and the native WebKit engine.
...
EOF
    ...
    cp "${RPM_TOPDIR}/RPMS/${RPM_ARCH}/${APP_NAME}-${VERSION}-1"*.rpm "MetaDesk-Fedora-${BUNDLE_ARCH}${ARTIFACT_SUFFIX}.rpm"
    rm -rf "${RPM_TOPDIR}"
    echo "Created: MetaDesk-Fedora-${BUNDLE_ARCH}${ARTIFACT_SUFFIX}.rpm"
fi
