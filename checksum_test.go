...
func TestParseChecksumFileGNUStyle(t *testing.T) {
	...
	got, err := parseChecksumFile(content, "MetaDesk.exe")
	if err != nil {
		t.Fatal(err)
	}
	if got != sum {
...
func TestParseChecksumFileBSDStyle(t *testing.T) {
	...
	content := "SHA256 (MetaDesk-Linux-x64.tar.gz) = " + sum + "\n"
	got, err := parseChecksumFile(content, "MetaDesk-Linux-x64.tar.gz")
	if err != nil {
		t.Fatal(err)
	}
	if got != sum {
...
func TestParseChecksumFileHandlesBinaryMarkerAndPaths(t *testing.T) {
	...
	content := sum + " *release-assets/MetaDesk.exe\n"
	got, err := parseChecksumFile(content, "MetaDesk.exe")
	if err != nil {
		t.Fatal(err)
	}
...
func TestParseChecksumFileRejectsMalformedDigest(t *testing.T) {
	if _, err := parseChecksumFile("nothex  MetaDesk.exe\n", "MetaDesk.exe"); err == nil {
...
func TestVerifyDownloadedChecksumAcceptsMatch(t *testing.T) {
	dir := t.TempDir()
	file := filepath.Join(dir, "MetaDesk.exe")
	...
	withStubTransport(t, hex.EncodeToString(sum[:])+"  MetaDesk.exe\n", 0)

	if err := verifyDownloadedChecksum(file, "MetaDesk.exe", "https://example.invalid/dl", true); err != nil {
...
func TestVerifyDownloadedChecksumRejectsMismatch(t *testing.T) {
	...
	withStubTransport(t, hex.EncodeToString(other[:])+"  MetaDesk.exe\n", 0)

	// A mismatch must be fatal even when checksums are not yet required.
	for _, required := range []bool{true, false} {
		err := verifyDownloadedChecksum(file, "MetaDesk.exe", "https://example.invalid/dl", required)
		if err == nil {
...
func TestVerifyDownloadedChecksumMissingSumsRespectsPolicy(t *testing.T) {
	...
	if err := verifyDownloadedChecksum(file, "MetaDesk.exe", "https://example.invalid/dl", false); err != nil {
...
func TestReleaseBaseURLForAsset(t *testing.T) {
	cases := []struct{ in, want string }{
		{"https://github.com/fajrisilmi12-cyber/MetaDesk/releases/latest/download/MetaDesk.exe",
			"https://github.com/fajrisilmi12-cyber/MetaDesk/releases/latest/download/"},
		{"https://github.com/fajrisilmi12-cyber/MetaDesk/releases/download/v1.5.9.2/MetaDesk-macOS-Universal.zip",
			"https://github.com/fajrisilmi12-cyber/MetaDesk/releases/download/v1.5.9.2/"},
		{"https://example.com/dl/MetaDesk-Linux-x64.tar.gz?token=abc",
...
func TestChecksumIsReadFromTheSameReleaseAsTheArtifact(t *testing.T) {
	...
	checksumHTTPTransport = roundTripFunc(func(req *http.Request) (*http.Response, error) {
		...
		return &http.Response{
			...
			Body:       io.NopCloser(strings.NewReader(hex.EncodeToString(sum[:]) + "  MetaDesk.exe\n")),
			Header:     make(http.Header),
			Request:    req,
		}, nil
	})
	...
	artifact := "https://github.com/fajrisilmi12-cyber/MetaDesk/releases/download/v1.5.9.2/MetaDesk.exe"
	if err := verifyDownloadedChecksum(file, "MetaDesk.exe", releaseBaseURLForAsset(artifact), true); err != nil {
		t.Fatalf("pinned release checksum must verify: %v", err)
	}
	want := "https://github.com/fajrisilmi12-cyber/MetaDesk/releases/download/v1.5.9.2/" + checksumFileName
	if requested != want {
