...
// Optional internal diagnostics. Enabled with METADESK_DEBUG=1; everything is a
...
func debugEnabled() bool {
	return os.Getenv("METADESK_DEBUG") == "1"
}

func debugLogPath() string {
	switch runtime.GOOS {
	case "darwin":
		return filepath.Join(homeDirOrDot(), "Library", "Application Support", "MetaDesk", "wa_debug.log")
	case "windows":
		return filepath.Join(os.Getenv("APPDATA"), "MetaDesk", "wa_debug.log")
	default:
		return filepath.Join(homeDirOrDot(), ".config", "metadesk", "wa_debug.log")
	}
}
