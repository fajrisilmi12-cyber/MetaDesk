...
func getInitScript(ua string) string {
	...
	try {
		...
		waRunModule('diagnostics-buffer', function() {
			...
			window.openIssueReporter = function(crashTail) {
				...
				var lines = ['MetaDesk v' + meta.ver + ' (' + meta.platform + ')', ''];
				if (waErrBuf.length) {
				...
				var url = 'https://github.com/fajrisilmi12-cyber/MetaDesk/issues/new' +
		...
		waRunModule('privacy-mode', function() {
			...
			var AUTO_LOCK_KEY = 'metadesk_privacy_autolock';
		...
		waRunModule('app-updater', function() {
			window.showUpdateBanner = function(latestVersion, releaseTitle, downloadUrl) {
				...
				var titleText = releaseTitle ? releaseTitle : ('MetaDesk v' + latestVersion);
				...
				try {
					if ((!window.isNotificationsEnabled || window.isNotificationsEnabled()) && window.sendNativeNotification) {
						...
						var notifBody = 'MetaDesk v' + latestVersion + ' is available. Click to update the application.';
				...
				if (window.checkForUpdateNative) {
					return window.checkForUpdateNative(true).then(function(res) {
						if (res && res.available) {
							...
							showFloatingToast('✅ MetaDesk is up to date (v' + cur + ')');
						}
						return res;
					}).catch(function() {
		...
		waRunModule('responsive-css', function() {
			...
			function injectResponsive() {
				...
				if (targetHead && !document.getElementById('metadesktop-responsive')) {
		...
		waRunModule('settings-modal', function() {
			...
			window.showSettingsModal = function() {
				...
				header.innerHTML = '' +
					...
					'  </div>' +
					'  <div>' +
					'    <h3 id="wa-modal-title" style="margin:0;font-size:15px;font-weight:600;">MetaDesk</h3>' +
				...
				cardAuto.innerHTML = '' +
					...
					'  </div>' +
					'  <div class="wa-text-muted" style="font-size:11px;">Automatically start MetaDesk on system login.</div>' +
					'</div>' +
					'<div style="display:flex;align-items:center;justify-content:flex-end;gap:12px;min-width:150px;flex-shrink:0;">' +
				...
				disclaimer.innerHTML = '<strong>MetaDesk</strong> is an independent application and is not affiliated with Meta.';
				...
				function refreshMediaPermissions() {
					...
					return Promise.all([
						...
						if (details) details.textContent = blocked ?
							'Permission is blocked by macOS. Open System Settings → Privacy & Security → Camera/Microphone, enable MetaDesk, then click Retry.' :
							'MetaDesk needs these permissions for calls and voice messages. If macOS asks, allow access and click Retry.';
					});
				}
		...
		waRunModule('emergency-settings', function() {
			...
			function showRecoveryPanel() {
				...
				card.innerHTML =
					'<strong style="font-size:14px;">MetaDesk — recovery</strong>' +
	...
	waRunModule('metadesk-dock', function() {
		try {
			if (document.getElementById('metadesk-dock')) return;
			...
			var cur = activeId();
			var items = [
				{id:'whatsapp', label:'WhatsApp (Ctrl+1)', text:'WA'},
				{id:'instagram', label:'Instagram (Ctrl+2)', text:'IG'},
				{id:'facebook', label:'Facebook (Ctrl+3)', text:'FB'}
			];
			items.forEach(function(it) {
				...
				var on = (it.id === cur);
				b.style.cssText = 'width:36px;height:36px;border-radius:10px;border:1px solid ' + (on ? '#00a884' : 'rgba(134,150,160,.4)') + ';background:' + (on ? '#00a884' : 'transparent') + ';color:' + (on ? '#fff' : '#aebac1') + ';font-size:12px;font-weight:700;cursor:pointer;';
				b.setAttribute('data-service', it.id);
			...
			function mount() {
				try {
					if (!document.body) return;
					if (!document.getElementById('metadesk-dock')) {
			...
			document.addEventListener('keydown', function(e) {
				try {
					...
					if (!mod) return;
					var id = null;
					if (e.key === '1') id = 'whatsapp';
					else if (e.key === '2') id = 'instagram';
