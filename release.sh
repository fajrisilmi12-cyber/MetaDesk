...
#
#   bash release.sh 1.0.0            build, checksum, publish
#   bash release.sh 1.0.0 --dry-run  build and checksum only, publish nothing
...
REPO="fajrisilmi12-cyber/MetaDesk"
VERSION=""
DRY_RUN=0
...
rm -f MetaDesk-Windows-x64.zip MetaDesk.exe
zip -j -q MetaDesk-Windows-x64.zip dist_win/MetaDesk.exe
cp dist_win/MetaDesk.exe MetaDesk.exe

ASSETS=(
  MetaDesk-Windows-x64-Setup.exe
  MetaDesk-Windows-x64.zip
  MetaDesk.exe
  MetaDesk-macOS-Universal.dmg
  MetaDesk-macOS-Universal.zip
)
...
| :--- | :--- | :--- |
| macOS 11.0+ (Apple Silicon & Intel) | `MetaDesk-macOS-Universal.dmg` | `MetaDesk-macOS-Universal.zip` |
| Windows 10/11 x64 (WebView2 runtime) | `MetaDesk-Windows-x64-Setup.exe` | `MetaDesk.exe` or `MetaDesk-Windows-x64.zip` |
...
(
  cd "$TMP"
  curl -sSL -O --max-time 180 "https://github.com/$REPO/releases/latest/download/MetaDesk-Windows-x64-Setup.exe"
