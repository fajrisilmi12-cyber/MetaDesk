...
func TestSaveDownloadedFile(t *testing.T) {
	...
	testContent := "MetaDesktop Light Media Test"
...
func TestValidateDownloadDirAcceptsNormal(t *testing.T) {
	...
	ok := []string{
		filepath.Join(home, "Downloads", "MetaDesk Downloads"),
		filepath.Join(home, "Downloads", "MetaDesk Downloads", "2026-01"), // monthly subfolder
		t.TempDir(),
	}
	for _, dir := range ok {
		if err := validateDownloadDir(dir); err != nil {
...
func TestValidateDownloadDirRootUser(t *testing.T) {
	...
	if err := validateDownloadDir("/root/Downloads/MetaDesk Downloads"); err != nil {
