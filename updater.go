...
const githubRepo = "fajrisilmi12-cyber/MetaDesk"
...
func findAssetForPlatform(release *GitHubRelease, goos, goarch string) *GitHubAsset {
	if goos == "darwin" {
		for _, preferred := range []string{"MetaDesk-macOS-Universal.zip", "MetaDesk-macOS.zip"} {
		...
		for _, a := range release.Assets {
			if strings.EqualFold(a.Name, "MetaDesk.exe") {
				return &a
			}
		}
		for _, a := range release.Assets {
		...
		for _, preferred := range []string{
			"MetaDesk-Linux-" + bundleArch + ".tar.gz",
			"MetaDesk-Linux-" + bundleArch + ".tar.gz",
		} {
			for i := range release.Assets {
				if strings.EqualFold(release.Assets[i].Name, preferred) {
...
func updateAssetForPlatform(goos, goarch string) string {
	switch goos {
	case "darwin":
		return "https://github.com/" + githubRepo + "/releases/latest/download/MetaDesk-macOS-Universal.zip"
	case "windows":
		return "https://github.com/" + githubRepo + "/releases/latest/download/MetaDesk.exe"
	case "linux":
		if goarch == "arm64" || goarch == "aarch64" {
			return "https://github.com/" + githubRepo + "/releases/latest/download/MetaDesk-Linux-arm64.tar.gz"
		}
		return "https://github.com/" + githubRepo + "/releases/latest/download/MetaDesk-Linux-x64.tar.gz"
	}
	return ""
}
...
// e.g. "MetaDesk-macOS-Universal.zip" names carry no version, so the
...
func fetchLatestVersionViaRedirect() (string, error) {
	...
	req.Header.Set("User-Agent", "MetaDesk-Updater")
...
set SRC=%s
set DST=%s
set UPDATE_LOG=%%TEMP%%\MetaDesk-update.log
...
:copy_failed
> "%%UPDATE_LOG%%" echo MetaDesk update could not replace "%%DST%%". The existing version was restarted.
start "" "%%DST%%"
goto cleanup
...
func isAllowedUpdateURL(rawURL string) bool {
	...
	// e.g. /fajrisilmi12-cyber/MetaDesk/releases/download/v1.2.3/...
	// or /fajrisilmi12-cyber/MetaDesk/releases/latest/download/...
	return strings.HasPrefix(strings.ToLower(u.Path), "/fajrisilmi12-cyber/metadesk/releases/")
}
...
func executeUpdate(ui UIController, downloadURL string) error {
	...
	if !isAllowedUpdateURL(downloadURL) {
		err := fmt.Errorf("update rejected: URL is not a MetaDesk GitHub release artifact")
		ui.Dispatch(func() {
	...
	destFile := filepath.Join(os.TempDir(), "metadesk_update_download"+ext)
