...
func applyUpdateDarwin(zipPath string) error {
	...
	extractDir := filepath.Join(os.TempDir(), "metadesk_update_extracted")
	_ = os.RemoveAll(extractDir)
	if err := unzipArchive(zipPath, extractDir); err != nil {
		return fmt.Errorf("unzip failed: %w", err)
	}

	// Look for MetaDesk.app inside extracted dir
	newAppPath := filepath.Join(extractDir, "MetaDesk.app")
	if _, err := os.Stat(newAppPath); err != nil {
		newAppPath = filepath.Join(extractDir, "MetaDesktop Light.app")
	}
	if _, err := os.Stat(newAppPath); err != nil {
		newAppPath = filepath.Join(extractDir, "MetaDesk.app")
	}
	...
	scriptPath := filepath.Join(os.TempDir(), fmt.Sprintf("metadesk_update_%d.sh", os.Getpid()))
	logPath := filepath.Join(os.TempDir(), "MetaDesk-update.log")
	script := `#!/bin/sh
set -eu
old_app="$1"
new_app="$2"
...
trap 'rm -f "$script_path"' EXIT
{
  ...
  if ! /usr/bin/ditto "$new_app" "$old_app"; then
    ...
    echo "MetaDesk update failed while copying the new app." > "$log_path"
    exit 1
  fi
